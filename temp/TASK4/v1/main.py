...
def create_test_case_data(case_id, position_before, position_after, orders_before, orders_after):
    ...
    for i, order in enumerate(orders_before, 1):
        data.append({
            'Case ID': '',  # Empty for orders in same case
            'State: before/after': '',  # Empty for orders in same state
...
def main():
    ...
    # Store after state (no need for deep copy as these are the final states)
    ...
    test_case_data = create_test_case_data(next_case_id, position_before, position_after, orders_before, orders_after)
