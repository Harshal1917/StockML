...
from callbacks import register_all_callbacks, CallbackType, position_update_callback, order_added_callback, order_modified_callback
import pandas as pd
...
def main():
    ...
    # position.add_callback(CallbackType.ORDER_MODIFIED, order_modified_callback)

    register_all_callbacks(position)
    position.add_callback(CallbackType.ORDER_MODIFIED, position.order_modified_callback)

    # Create Order instances
    order1 = Order(trade_id=111, pending_qty=0, executed_qty=1500, 
                status='executed', order_side='buy', symbol='TCS')
    order2 = Order(trade_id=111, pending_qty=100, executed_qty=100, 
                  status='partially_executed', order_side='buy', symbol='TCS')
    order3 = Order(trade_id=111, pending_qty=0, executed_qty=200, 
                  status='executed', order_side='sell', symbol='TCS')
    order4 = Order(trade_id=111, pending_qty=500, executed_qty=0,
                  status='pending', order_side='sell', symbol='TCS')
    order5 = Order(trade_id=111, pending_qty=200, executed_qty=100, 
                  status='partially_executed', order_side='sell', symbol='TCS')

    # Register callbacks for each order
    ...
    position.debug_print_state()
    
    # Modify order4 (ORDER-4)
    order4.modify_order_execute_qty(200)
    ...
    orders_after = [order1, order2, order3, order4, order5]
    ...
    # # Register callback for the independent order
    # independent_order.add_callback(CallbackType.ORDER_MODIFIED, order_modified_callback)

    # # Modify the order - this will trigger the callback even without a position
    # independent_order.modify_order_execute_qty(200)

    # # Later, if you add it to a position, the same callback will work with position functionality
    # position.add_order(independent_order)
    # independent_order.modify_order_execute_qty(100)  # Now will also trigger position countering
