#define parameters to evaluate the model - [IC,IR,AR,ICIR]

#how to work on custom data from yfinance to qlib
#train workflow benchmarks!
#https://youtu.be/Hb1l70-at9s?si=kO_HCTaFMYBg2_vm: watch this video to understand new technique


#stretagies, backtest


#### 3. Data Processing- Outlier Removal: Use robust normalization or remove extreme outliers in features and labels.- Feature Selection: Remove highly correlated or irrelevant features to reduce noise.
#### 4. Model Tuning- Hyperparameter Optimization: Use grid search or Bayesian optimization to tune LightGBM parameters.- Try Other Models: Test other models (XGBoost, CatBoost, neural networks, etc.) for comparison.
#### 5. Validation & Backtesting- Cross-Validation: Use time-series cross-validation to ensure robustness.- Walk-Forward Analysis: Simulate real trading by retraining and testing in rolling windows.
#### 7. Ensemble Methods - Combine predictions from multiple models to reduce variance and improve stability.
...
task = {
    ...
    "dataset": {
        ...
        "kwargs": {
            "handler": {
                "class": "Alpha158",  # <-- Use Alpha158 handler
...
def test_model(model, dataset, exp_id):
    ...
    # Use the correct label for Alpha158
    true = test_data[("label", "LABEL0")]
    ...
    #// ... existing code ...
    ic = calc_ic(results["actual"], results["predicted"])
    returns = results["actual"]  # or use predicted returns for strategy
    ir = calc_ir(returns)
    ar = calc_ar(returns)
    icir = calc_icir(results["actual"].rolling(20).corr(results["predicted"]))
    print(f"IC: {ic:.4f}, IR: {ir:.4f}, AR: {ar:.4f}, ICIR: {icir:.4f}")
    #// ... existing code ...
    ...
    ax1.set_title("Actual vs Predicted Returns", fontsize=14)
